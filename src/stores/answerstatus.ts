...
export const useAnswerStatusStore = defineStore('answerstatus', () => {
  const status = ref<{ isCorrect: boolean | null; questionId: number }[] | undefined>(undefined)

  //問題数の数だけundefinedを入れた配列を作成
  function initStatus(length: number) {
    status.value = Array(length)
      .fill(null)
      .map((_, i) => {
        return { isCorrect: null, questionId: i + 1 }
      })
  }
  ...
  function setStatus(index: number, value: boolean) {
    ...
    if (status.value) {
      status.value[index]['isCorrect'] = value
    } else {
